...
export function SignupForm({ onSuccess, onSwitchToLogin, signupMutation }: SignupFormProps) {
  ...
  const onSubmit = async (values: SignupFormValues) => {
    ...
    try {
      ...
      setError(e?.response?.data?.detail || 'Signup failed. This email may already be registered.');
    }
  };
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 0 }}>
      ...
      <div style={{ display: 'flex', gap: 10 }}>
        {[
          { Icon: GoogleIcon, label: 'Google', href: 'http://localhost:8000/auth/google?action=signup' },
          { Icon: GitHubIcon, label: 'GitHub', href: 'http://localhost:8000/auth/github?action=signup' },
        ].map(({ Icon, label, href }) => (
          <motion.button
            key={label}
            type="button"
